...
export default function Dashboard({ onNavigate }) {
    ...
    // Each spec row has a stable uid to avoid index-key bugs when rows are deleted
    const [specRows, setSpecRows] = useState([]);
    const nextSpecId = useRef(0);
    const makeSpecRow = (key = '', value = '') => ({ uid: nextSpecId.current++, key, value });
    ...
    const handleLoadMore = async () => {
        if (!currentShop || loadingMore || products.length >= totalCount) return;
        try {
            setLoadingMore(true);
            const res = await databases.listDocuments(DATABASE_ID, PRODUCTS_COLLECTION_ID, [
                Query.equal('shopId', currentShop.$id),
            ...
            setProducts(prev => [...prev, ...res.documents]);
            setTotalCount(res.total);
        } catch (err) {
    ...
    const handleUpdateQuantity = async (product, delta) => {
        ...
        try {
            ...
            // Use functional updater to avoid stale closure with rapid clicks
    ...
    const openEditProductModal = (product) => {
        ...
        if (product.specifications) {
            try {
                ...
                specs = Object.entries(parsed).map(([key, value]) => makeSpecRow(key, value));
            } catch (e) {}
        }
        setSpecRows(specs);
    ...
    const handleCategoryChange = (e) => {
        ...
        if (!hasEnteredValues && CATEGORY_SPECS[cat]) {
            setSpecRows(CATEGORY_SPECS[cat].map(key => makeSpecRow(key, '')));
        }
    };
    ...
    const handleSaveProduct = async (e) => {
        ...
        try {
            ...
            // Clean up removed images from storage (only after successful document write)
            if (editingProduct) {
            ...
            // Rollback: delete any files that were uploaded before the document save failed
            for (const fileId of newUploadedIds) {
    ...
    const getShareText = (p) => {
        ...
        const catalogUrl = currentShop?.slug
            ? `${window.location.origin}/shops/${currentShop.slug}`
        ...
        <div className="dashboard-page">
            ...
            <main className="dash-content">
                ...
                <div className="stats-row" style={{ gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', marginBottom: '1.25rem' }}>
                    <div className="stat-card available" style={{ background: 'rgba(16, 185, 129, 0.08)', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                        <div className="stat-value" style={{ color: '#059669' }}>{availableCount}</div>
                        <div style={{ fontSize: '0.85rem', color: '#047857', fontWeight: 600 }}>Available</div>
                    </div>
                    <div className="stat-card oos" style={{ background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                        <div className="stat-value" style={{ color: '#dc2626' }}>{outOfStockCount}</div>
                        <div style={{ fontSize: '0.85rem', color: '#b91c1c', fontWeight: 600 }}>Out of Stock</div>
                    </div>
                </div>
                ...
                <div id="product-list" style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                    {loading ? (
                        <div className="empty-state" style={{ textAlign: 'center', padding: '3.5rem 1rem', background: '#fff', borderRadius: 'var(--dash-radius)', border: '1px solid var(--dash-border)' }}>
                            <div style={{ width: '36px', height: '36px', border: '3px solid var(--dash-border)', borderTopColor: 'var(--dash-accent)', borderRadius: '50%', animation: 'spin 0.7s linear infinite', margin: '0 auto' }}></div>
                            <p style={{ color: 'var(--dash-text-secondary)', fontSize: '0.85rem', marginTop: '1rem', fontWeight: 500 }}>Loading inventory...</p>
                        </div>
                        ...
                        <>
                            {filteredProducts.map(product => {
                                ...
                                const cardBg = isOos ? 'rgba(239, 68, 68, 0.04)' : 'rgba(16, 185, 129, 0.03)';
                                const cardBorder = isOos ? 'rgba(239, 68, 68, 0.25)' : 'rgba(16, 185, 129, 0.25)';
                            ...
                            {products.length < totalCount && (
                                <div style={{ textAlign: 'center', marginTop: '1rem' }}>
                                    <button
                                        ...
                                        style={{ padding: '0.65rem 1.5rem', fontSize: '0.875rem', fontWeight: 600, borderRadius: 'var(--radius-md)' }}
                                    >
                                        {loadingMore ? 'Loading More...' : `Load More Products (Showing ${products.length} of ${totalCount})`}
                                    </button>
                                </div>
                            )}
                        </>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    ...
                                    <div>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setSpecRows([...specRows, makeSpecRow()])}
                                        ...
                                        {specRows.map((row, idx) => (
                                            <div key={row.uid} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.4rem' }}>
                                                <input
                                                    type="text"
                                                    placeholder="Spec"
                                                    value={row.key}
